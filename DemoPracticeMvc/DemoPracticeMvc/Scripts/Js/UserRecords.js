$(document).ready(function () {
    ...
    $("#FirstNameSortingLink").click(function () {
        UserDataFill("Order By FirstName", "");
    });

    $("#LastNameSortingLink").click(function () {
        UserDataFill("Order By LastName", "");
    });

    $("#EmailIdSortingLink").click(function () {
        UserDataFill("Order By EmailId", "");
    });

    $("#GenderSortingLink").click(function () {
        UserDataFill("Order By Gender", "");
    });

    $(document).on('click', '.EditRecordCLS', function () {
        UserDataFill($(this).attr('data-UserIdEdit').trim(), "Where Id=");
    });
    ...
    $("#TxtFNameSearch").keyup(function () {
        UserDataFill("", "Where FirstName like '%" + $(this).val().trim() + "%'");
    });
    $("#TxtLNameSearch").keyup(function () {
        UserDataFill("", "Where LastName like '%" + $(this).val().trim() + "%'");
    });
    $("#TxtContactNoSearch").keyup(function () {
        UserDataFill("", "Where ContactNo like '%" + $(this).val().trim() + "%'");
    });
    $("#TxtEmailIdSearch").keyup(function () {
        UserDataFill("", "Where EmailId like '%" + $(this).val().trim() + "%'");
    });
...
function UserDataFill(OrderBy, WhereClause) {
                ...
                if (WhereClause == 'Where Id=') {
                    ...
                    for (var i = 0; i < UserArr.length; i++) {
                        ...
                        $("#UserTableBody").append(
                            ...
                            + "<td class='text-center'><a data-UserStatusId=" + UserArr[i].Id + " data-UserStatus=" + UserArr[i].Status + " class='EditStatusCLS'>" + StatusIcon + "</a></td>"
                            + "</tr>"
                        );
...
function UpdateUserStatus(Id, Status) {
    ...
    $.ajax({ //Declaration of ajax function using ajax keyword
        url: "/Index/UpdateUserStatus", //passsing the name of get or post method
